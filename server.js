 ...
 * - Remove rota duplicada /api/financeiro-dashboard
 * - Remove uso de authenticateToken (não definido) que derrubava o Node e causava 503
 * - Mantém a versão "completa" do /api/financeiro-dashboard (por planosReceber/planosPagar)
 * - Ajusta log do listen para ambiente de hospedagem
 */
...
// =========================
// LOGICA DE FERIADOS E DIAS ÚTEIS
// =========================
...
const getProximoDiaUtil = (dataInput) => {
  ...
  // Ajuste para evitar problemas de fuso horário local na conversão (strings sem "T")
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      try {
        ...
        return res.json({ success: true, require2fa: true, email });
      } catch (mailErr) {
        console.error('Erro ao enviar email:', mailErr);
        return res.status(500).json({ success: false, message: 'Erro envio email.' });
      }
    }
    ...
    return res.status(500).json({ success: false, message: 'Erro BD.' });
  }
});
  ...
  try {
    const [tokens] = await pool.query(
      `SELECT * FROM tokens_acesso
       WHERE email = ? AND token = ? AND expira_em > NOW()
    ...
    if (tokens.length > 0) {
      ...
      return res.json({
        success: true,
        user: { ...u, Nome: u?.Nome || 'Usuário', Role: u?.Role || 'user' },
      });
    }
...
app.post('/api/definir-senha', async (req, res) => {
  ...
  try {
    await pool.query('UPDATE usuarios SET Senha = ?, Senha_prov = NULL WHERE Email = ?', [
      novaSenha,
      email,
    ]);
...
app.get('/api/orcamento', async (req, res) => {
  ...
  try {
    ...
    if (ano) {
      queryReal += ' AND (Ano = ? OR Ano = ?)';
      paramsReal.push(ano, parseInt(ano, 10) + 1);
    }
    ...
    resRealRaw.forEach((r) => {
      ...
      // Boletos: próximo dia útil muda competência (mês/ano)
      ...
      // Só joga no mapa se (após postergação) pertence ao ano filtrado
    ...
    orcamentoData.forEach((row) => {
      ...
      chavesFrontend.forEach((chaveFront, index) => {
        ...
        dadosMesesItem[chaveFront] = { orcado: valOrcado, realizado: valRealizado, diferenca };
    ...
    // Ordenação crescente numérica
    Object.values(grupos).forEach((grupo) => {
      grupo.detalhes.sort((a, b) =>
        a.conta.localeCompare(b.conta, undefined, { numeric: true, sensitivity: 'base' })
      );
    });
    ...
    console.error(e);
    return res.status(500).json({ error: 'Erro ao processar orçamento' });
  }
});
...
app.get('/api/financeiro-dashboard', async (req, res) => {
  try {
    ...
    ['receber', 'pagar'].forEach((k) => {
      const g = grupos[k];
      const detalhes = Object.values(g.planosMap).sort((a, b) =>
        a.conta.localeCompare(b.conta, undefined, { numeric: true, sensitivity: 'base' })
      );
...
app.get('/api/dashboard', async (req, res) => {
  try {
    ...
    let query =
      'SELECT Origem_DFC, Nome_2, Codigo_plano, Nome, Mes, Ano, Valor_mov, Natureza, Dt_mov, Baixa FROM dfc_analitica WHERE 1=1';
    const params = [];

    // Buscamos um ano antes para capturar boletos de 31/12 que pulam para 01/01
    if (view !== 'anual' && ano) {
      query +=
        ' AND (Ano = ? OR ( (Nome LIKE "%BOLETO%" OR Nome LIKE "%CARTÕES (DÉBITO E CRÉDITO)%") AND Ano = ?))';
      params.push(ano, parseInt(ano, 10) - 1);
    }
    ...
    rawData.forEach((row) => {
      ...
      let chaveColuna = '';
      if (view === 'anual') chaveColuna = numAno.toString();
      else if (view === 'trimestral') chaveColuna = `Q${Math.ceil(numMes / 3)}`;
      else chaveColuna = mapaMeses[numMes];
      ...
      if (row.Origem_DFC) {
        ...
        const tituloGrupo = configCategorias[chaveBanco];
        ...
        if (tituloGrupo) {
          ...
          if (!grupo.subgruposMap[nome2].itensMap[itemChave]) {
            grupo.subgruposMap[nome2].itensMap[itemChave] = { conta: itemChave, ...zerarColunas(), tipo: 'item' };
          }
