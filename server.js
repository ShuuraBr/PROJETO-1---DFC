...
// --- CONFIGURAÇÃO DO E-MAIL (SMTP Hostinger) ---
...
// =========================================================================
// ROTAS DE AUTENTICAÇÃO (LOGIN COM 2FA)
// =========================================================================

// PASSO 1: Verifica senha e envia token (30 SEGUNDOS PELO BANCO DE DADOS)
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        if (rows.length > 0) {
            // --- GERAÇÃO DO TOKEN 2FA ---
            const token = crypto.randomInt(100000, 999999).toString();
            
            // CORREÇÃO CRÍTICA: Usamos o horário do MySQL (NOW) para somar 30 segundos.
            // Isso evita erro de "Expirado" se o horário do Node.js estiver diferente do Banco.
            await pool.query(
            ...
            try {
                ...
                // Retorna sucesso PARCIAL, pedindo o token
                res.json({ success: true, require2fa: true, email: email });

            } catch (mailErr) {
                console.error("Erro ao enviar email:", mailErr);
                // Retorna erro para o usuário saber que o email falhou
                res.status(500).json({ success: false, message: 'Senha correta, mas falha ao enviar o e-mail.' });
            }
...
// PASSO 2: Valida o token e retorna o usuário final
app.post('/api/validar-token', async (req, res) => {
    const { email, token } = req.body;

    try {
        // Busca token válido (expira_em > NOW() usa o relógio do banco)
...
app.get('/api/orcamento', async (req, res) => {
    ...
    try {
        ...
        orcamentoData.forEach(row => {
            const codigo = row.Plano; const nome = row.Nome; const depto = row.Departamento1 || 'Sem Departamento'; const contaFormatada = `${codigo} - ${nome}`;
            if (!grupos[depto]) { grupos[depto] = { conta: depto, tipo: 'grupo', dados: {}, detalhes: [] }; chavesFrontend.forEach(k => grupos[depto].dados[k] = { orcado: 0, realizado: 0, diferenca: 0 }); }
            const dadosMesesItem = {};
            chavesFrontend.forEach((chaveFront, index) => {
                const nomeColunaBanco = colunasBanco[index]; const mesNumero = index + 1;
                let valOrcado = parseFloat(row[nomeColunaBanco]) || 0; if (ocultarOrcado) valOrcado = 0;
                const valRealizado = mapRealizado[`${codigo}-${mesNumero}`] || 0; const diferenca = valOrcado - valRealizado;
                dadosMesesItem[chaveFront] = { orcado: valOrcado, realizado: valRealizado, diferenca: diferenca };
                grupos[depto].dados[chaveFront].orcado += valOrcado; grupos[depto].dados[chaveFront].realizado += valRealizado; grupos[depto].dados[chaveFront].diferenca += diferenca;
            });
            grupos[depto].detalhes.push({ conta: contaFormatada, tipo: 'item', dados: dadosMesesItem });
        });
        res.json(Object.values(grupos));
    } catch (e) { console.error("Erro Orçamento:", e); res.status(500).json({ error: 'Erro ao processar orçamento' }); }
});
